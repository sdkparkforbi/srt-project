...
### 가장 중요한 발견은요?
국내 은행의 기업 평균위험가중치가 36~51%로, 흔히 가정하는 75%의 절반 수준이었어요. 위험가중치가 이미 낮으면 선순위는 하한(15%) 근처라 SEC-SA와 SEC-IRBA 결과가 비슷해지고, 자본경감의 크기는 결국 '우선손실 트렌치를 얼마나 두껍게(예상손실에 맞춰) 잡느냐'와 '얼마나 이전하느냐'가 좌우해요.

### 데이터는 몇 개 은행인가요?
12개 은행 중 9개는 공시에서 직접 측정했어요(KB·신한·하나·우리·IBK·NH·KDB·수협·부산). 경남·광주는 상세표 추출이 안 돼 동종 평균(42%)으로 추정했고, 제주는 표준방법이라 내부등급법 데이터가 없어요. 부산은 30쪽 근방 신용리스크 표에서 평균위험가중치 48.5%로 확인했어요.
